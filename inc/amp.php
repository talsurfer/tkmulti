 ...
 */
function tkm_amp_class( $default, $active, $state ) {
	$output = '';
	if( tkm_is_amp() ) {
		$output .= sprintf(
			' [class]="%s"',
 ...
 */
function tkm_amp_toggle( $state = '', $disable = array() ) {
	if( ! tkm_is_amp() )
		return;
	$settings = sprintf(
		'%1$s: ! %1$s',
 ...
 */
function tkm_amp_nav_dropdown( $theme_location = false, $depth = 0 ) {
	$key = 'nav';
	...
	if( 1 < $depth )
		$key .= 'Depth' . $depth;
	return tkm_amp_toggle( $key ) . tkm_amp_class( 'submenu-expand', 'expanded', $key );
}
